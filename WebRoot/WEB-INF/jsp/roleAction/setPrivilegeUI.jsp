		...
		<script type="text/javascript">
			...
			$(function(){
				$('input[name=privilegeIds]').click(function(){
					...
					//当选中子元素时  他的直系父辈元素都被选中
					if(this.checked){
						$(this).parents("li").children("input[name=privilegeIds]").attr("checked",true);
					}else{// 当取消某个元素时如果同级元素都没有被选中 则上级元素也被取消
						if($(this).parent().siblings("li").children("input:checked").size() == 0){
							$(this).parent().parent().siblings("input[name=privilegeIds]").attr("checked",false);
						}
					}
						...
						<table cellpadding="0" cellspacing="0" class="mainForm">
							...
							<tbody id="TableData">
								<tr class="TableDetail1">
									<!-- 显示权限树 -->
<td>
<%-- 使用struts2 标签显示复选框  有回显效果  但是无法修改过
	<s:checkboxlist name="privilegeIds" list="privilegeList" listKey="id" listValue="name"></s:checkboxlist><br/>
--%>
<%--自己循环显示复选框  需要自己实现回显功能
