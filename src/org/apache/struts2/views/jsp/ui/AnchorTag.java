...
//			throw new IllegalStateException("需要登录用户才能操作!");
			action = "loginOut_loginUI";
			return 0;
		}
		//获取所需要权限的url  
		String privUrl = "/"+action;
		//处理后面的参数 和UI
		if(privUrl.indexOf('?')>-1){
			privUrl = privUrl.substring(0,privUrl.indexOf('?'));
		}
		if(privUrl.endsWith("UI")){
			privUrl = privUrl.substring(0,privUrl.length()-2);
		}
		
		if(user.hasPrivilegeByUrl(privUrl)||action.equals("loginOut_logOut")||action.equals("loginOut_loginUI")){
			return super.doEndTag();//输出a标签  继续执行后面的代码
		}else{
			return BodyTagSupport.EVAL_PAGE;//不输出a 继续执行后面的代码
		}
	}
